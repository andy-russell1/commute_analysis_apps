...
def _office_labels(offices: Sequence[dict]) -> dict[str, tuple[str, str]]:
    ...
    for office in offices:
        office_id = str(office.get("officeID", ""))
        full = str(office.get("address", "")).strip()
        short = (full.split(",")[0].strip() if full else office_id) or office_id
        out[office_id] = (short, full or office_id)
    return out
...
def office_stats(
    ...
    *,
    ...
    for office in offices:
        ...
        n = int(vals.size)
        short, full = labels.get(office_id, (office_id, office_id))
        rows.append(
            {
                "officeID": office_id,
                "Office ID": office_id,
...
def threshold_bands(
    ...
    *,
    ...
    for office in offices:
        ...
        short, _full = labels.get(office_id, (office_id, office_id))
        rows.append(
            {
                "Office": short,
                "Total Employees": n,
...
def wide_table(df_valid: pd.DataFrame, office_id: str, methods: Sequence[str]) -> pd.DataFrame:
    ...
    return out.rename(
        columns={"employeeID": "Employee ID", "postcode": "Postcode", "city": "City", "country": "Country"}
    )


def wide_table_all_offices(df_valid: pd.DataFrame, offices: Sequence[dict], methods: Sequence[str]) -> pd.DataFrame:
    _ensure_cols(df_valid, ["employeeID", "officeID", "method", "travel_time_min"])
    d = df_valid.copy()
    d["travel_time_min"] = _as_float(d["travel_time_min"]).round(1)
    ...
    office_lookup = {str(o.get("officeID", "")): str(o.get("address", "")).split(",")[0].strip() for o in offices}
    d["office_short"] = d["officeID"].astype(str).map(office_lookup)
    meta = d[["employeeID", "postcode", "city", "country"]].drop_duplicates(subset=["employeeID"]).copy()
    piv = (
        d[d["method"].astype(str).isin([str(m) for m in methods])]
        .pivot_table(index="employeeID", columns=["office_short", "method"], values="travel_time_min", aggfunc="min")
        .reset_index()
    )
    ...
    return out.sort_values("Employee ID")
