...
def match_columns(df: pd.DataFrame) -> Cols:
    ...
    method = _find_col(df, "Query Transport Method", "Transport Method", "transport method", "method", "transport")
...
def compute_office_summary(df_valid: pd.DataFrame) -> pd.DataFrame:
    ...
    d["officeShort"] = d["officeShort"].mask(d["officeShort"].eq(""), d["officeID"].astype(str))
    grouped = d.groupby(["officeID", "officeShort"], dropna=False)
    summary = grouped["travel_time_min"].agg(["count", "median", "mean"]).reset_index()
    summary = summary.rename(
        columns={"officeShort": "Office", "count": "Employee Count", "median": "Median (mins)", "mean": "Mean (mins)"}
    )
        ...
        rows.append(
            {
                "Office": office,
                "Total Employees": n,
                "<=30 min": int(np.sum(vals <= t1)) if n else 0,
                "30-45 min": int(np.sum((vals > t1) & (vals <= t2))) if n else 0,
                "45-60 min": int(np.sum((vals > t2) & (vals <= t3))) if n else 0,
                ">60 min": int(np.sum(vals > t3)) if n else 0,
            }
        )
