    ...
    for macro in updated_major["macro"].dropna().unique():
        with st.expander(f"Macro: {macro}", expanded=False):
            ...
            if st.button(f"Normalize Majors in {macro}", key=f"{key_prefix}_major_norm_{_safe_key(str(macro))}"):
                _normalize_slider_values([(slider_key, input_key) for _, _, slider_key, input_key in major_slider_meta])
...
def render_page() -> None:
    ...
    if mode == MODE_CLIENT:
        st.info("Client mode focuses on decision outputs. Direction overrides stay in Advanced mode.")
